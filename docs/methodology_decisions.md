...
| D-008 | Formal regimes are NBER recession versus non-recession; 2008, 2020, and 2022 are separate case studies. | Locked 2026-08-10 | Avoids treating economically distinct episodes as one homogeneous regime. |
| D-009 | Primary long-history exact-index OHLC candidate is Yahoo Finance `^GSPC` with `auto_adjust=False`; FRED `SP500` close validates overlap; SPY remains engineering-only. | Locked 2026-08-10 | W2-06 must create immutable snapshots, hashes, manifests, discrepancy reports, and software-version provenance. |
...
| O-001 | Map the free Cboe historical CSV `CLOSE` field to an exact daily timestamp/convention across history. | Open before target construction | Current primary documents support a last RTH value near 4:15 p.m. ET, but do not explicitly bind every free-history CSV observation to that timestamp. SPX cash close is normally 4:00 p.m. ET. |
...
- Dimensional checks distinguish variance from volatility and percentage points
  from decimals.
- Raw snapshots, SHA-256 hashes, schema/coverage/missingness, software versions,
