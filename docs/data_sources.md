...
**Status:** Hierarchy and feasibility complete; W2-06 implementation is in
review because validation discrepancies and the local TLS workaround require
...
|---|---|---|---|
| VIX history | Cboe historical CSV | Primary | Issuer source with long public history. Exact daily `CLOSE` timestamp across the free history remains open before panel construction. |
| S&P 500 OHLC | Yahoo Finance `^GSPC` through pinned `yfinance`, `auto_adjust=False` | Primary long-history candidate | Exact price index rather than SPY; supplies OHLC. Yahoo/yfinance is not an exchange-authoritative feed, so provenance and validation are mandatory. |
...
| Short risk-free rate | U.S. Treasury daily yield curve | Separate, non-blocking | Needed for other project components, not for W2-06 exact-index OHLC acquisition. |

## W2-06 retrieval contract
...
- Yahoo and FRED source files are stored under the UTC retrieval timestamp, written
  immutably, and hashed with SHA-256. The manifest records schema, coverage,
  missingness, byte counts, source URLs, and validation statistics.
- FRED overlap is matched by date. Values differing by more than one index-point
  cent are counted and sampled in the manifest; Yahoo OHLC remains unchanged.
- Raw files stay ignored by Git because redistribution rights differ. Code,
  documentation, and compact metadata are the reproducible deliverables.
...
discrepancy is evidence for source review; it is not permission to edit raw
prices. A licensed index feed remains the escalation path if validation exposes
...
| Yahoo usable coverage | 9,217 rows, 1990-01-02 through 2026-08-07 |
...
(0.02), and 2020-10-06 (0.02). These rows remain unchanged in the Yahoo raw
snapshot and must be reviewed during W2-07 cleaning; the two largest are
...
snapshot used `verify=False` for Yahoo and records that fact in the manifest;
FRED was fetched with Windows curl and its certificate/revocation handling.
