...
the cleaned panel or any realized-variance estimate exists; cleaning and
realized-variance implementation remain separate future tasks.
...
| `sp500_open` | float | index points | Yahoo `^GSPC`, `auto_adjust=False` | observed after open | Exact index, not SPY; provider values are preserved without correction in the normalized acquisition snapshot. |
...
| `vix_close_pct` | float | annualized percentage points | Cboe VIX daily `CLOSE` | end-of-day | Treated operationally as an end-of-day predictor for date \(t\); the target begins with the first return ending strictly after \(t\). Exact historical timestamp remains a provenance limitation. |
